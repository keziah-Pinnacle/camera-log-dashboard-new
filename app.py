...
# Color function for charging states
def get_color(is_start=True):
    return 'green' if is_start else 'red'  # Green for start, red for stop
...
if len(uploaded_files) > 0:
    ...
    if not all_data:
        ...
        if not charging_df.empty:
            ...
            # Identify charging sessions and stops
            ...
            for _, group in charging_groups.iterrows():
                ...
                fig1.add_trace(go.Bar(
                    ...
                    customdata=[group['start_time_str'], group['duration_hours'], group['start_battery'], group['end_battery']]
                ))
            ...
            st.text("This graph shows start (green) and stop (red) times of charging sessions. Hover for details.")
        ...
        if not power_df.empty:
            ...
            st.text("This graph shows exact power on (green) and power off (red) times per day.")
